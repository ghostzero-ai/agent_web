...
- 当前阶段：Mobile M0.2 已完成，M0.3 卓易通真机验证待执行
...
- APK 冷启动、后台恢复和进程被系统终止后的重启。
- Tailscale 在线/离线提示与服务端不可用兜底。
- Chat SSE 流式输出、停止生成与树形分支。
- Markdown、表格、行内/块级公式。
- API Provider 设置与凭据保存。
- 文件导出、系统分享和下载目录行为。
- 音频播放、暂停、耳机与锁屏行为。
- 通知权限、精确提醒设置和重启后恢复。
- 卓易通环境下 HMS Push Token、后台推送和点击深链。

最后三组能力目前只有插件/架构预留，不应在真机测试前标记为完成。
